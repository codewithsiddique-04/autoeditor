...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  const { clips, fps = 30, transitions = [], transitionDuration = 0.4, motions = [], motionAmount = 0.08, audioFile = null,
    ...
    effectId = "none", effectIntensity = 0.5, effectByName = {}, sfx = [],
  ...
  const wantAudio = !!audioCodec && (!!voiceSource || ((clipAudio.length + sfxList.length) > 0 && !!OAC));
  ...
  // Decode each placed sound effect and place the whole buffer at its time. Same
  // { chans, at, vol } shape as clip audio, so pumpAudioTo mixes them unchanged.
  async function prepareSfxMixes() {
    ...
    for (const s of sfxList) {
      ...
      try {
        ...
        buf = await dctx.decodeAudioData(await res.arrayBuffer());
      } catch (_) { buf = null; }
      if (!buf) continue;
      const nCh = Math.min(mixChannels, buf.numberOfChannels);
      const chans = [];
      for (let c = 0; c < nCh; c++) chans.push(Float32Array.from(buf.getChannelData(c)));
  ...
  if (wantAudio) {
    ...
    try { clipMixes = clipMixes.concat(await prepareSfxMixes()); } catch (_) { /* keep clip audio */ }
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    // A per-clip override wins over the global effect for the current clip.
    const fxOv = clip && effectByName[clip.name];
    applyEffect(ctx, fxOv ? fxOv.id : effectId, fxOv ? fxOv.intensity : effectIntensity, W, H, t);
